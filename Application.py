...
class Application(Frame):
    ...
    def clear(self, event):
        event.widget.selection_range(0,
                                     END,
                                    )
    ...
    def renameTask(self):
        ...
        if check1 == 1 and check3 == 1 and check5 == 1:         
            zCnt = len( str(len(self.filesRenameList)) ) + 1 # rename using filename of all types and selected files only
            cnt = 1
            ...
            fileList = self.fileManager.getDirFiles(self.dir)  #rename using filename of all types on all files 
            ...
            fileListToRename = [] #rename using filename of selected file types on selected files only
            ...
            fileList = self.fileManager.getDirFiles(self.dir)  #rename using filename of selected file types on all files
            fileListToRename = []
            ...
            regex = self.userInputRegexExp   #rename using regex of all file types on selected files only
            ...
            regex = self.userInputRegexExp  #rename using regex of all file types  on all files
            matchingFiles = []
            ...
            regex = self.userInputRegexExp  #rename using regex of selected file types on selected files
            matchingFiles = []
            ...
            regex = self.userInputRegexExp  #rename using regex of selected file types on all files
            matchingFiles = []
            fileList = self.fileManager.getDirFiles(self.dir)
